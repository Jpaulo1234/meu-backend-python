...
async def convert_docx_to_pdf(file: UploadFile = File(...)):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            # LibreOffice headless mode for perfect DOCX to PDF conversion
            subprocess.run([
...
async def crack_pdf(file: UploadFile = File(...)):
    ...
    try:
        # Try common passwords and numeric pins
        ...
        if found_password is not None:
            ...
            raise HTTPException(status_code=400, detail="Senha muito complexa. A quebra de senha online suporta apenas PINs de 4 dígitos ou senhas muito comuns por limitações de tempo.")
    finally:
        pass
